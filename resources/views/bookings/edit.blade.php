<x-app-layout>
    ...
    <div class="py-8 max-w-2xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white p-6 shadow rounded-xl border border-gray-100">
            ...
            <form method="POST" action="{{ route('bookings.update', $booking) }}" id="booking-form">
                @csrf
                @method('PUT')
                ...
                <div class="mb-5">
                    ...
                    <div id="calendar"></div>
                    <p class="text-xs text-gray-400 mt-2">Red / strikethrough dates are already booked (excluding this booking).</p>
                    <p class="text-sm font-medium text-indigo-600 mt-2" id="selected-dates-display"></p>
                </div>

                <input type="hidden" name="check_in" id="check_in" value="{{ old('check_in', $booking->check_in->format('Y-m-d')) }}">
                <input type="hidden" name="check_out" id="check_out" value="{{ old('check_out', $booking->check_out->format('Y-m-d')) }}">

                @if (auth()->user()->isAdmin())
                    <div class="mb-5">
                        <label class="block mb-1 font-medium text-sm text-gray-700">Status</label>
                ...
                <div class="mb-6">
                    ...
                    <textarea name="notes" rows="3" class="w-full border-gray-300 rounded-lg focus:border-indigo-500 focus:ring-indigo-500">{{ old('notes', $booking->notes) }}</textarea>
                </div>
    ...
    <script>
        let calendar = null;
        let bookedDates = [];
        const currentCheckIn = "{{ $booking->check_in->format('Y-m-d') }}";
        const currentCheckOut = "{{ $booking->check_out->format('Y-m-d') }}";
        ...
        function initCalendar() {
            ...
            calendar = flatpickr("#calendar", {
                ...
                minDate: "today",
                disable: bookedDates,
                defaultDate: [currentCheckIn, currentCheckOut],
            ...
            document.getElementById('selected-dates-display').innerText =
                `Selected: ${currentCheckIn} to ${currentCheckOut}`;
        }
        ...
        function loadBookedDates(roomId) {
            fetch(`/rooms/${roomId}/booked-dates`)
                .then(res => res.json())
                .then(dates => {
                    bookedDates = dates.filter(d => d !== currentCheckIn); // don't block the booking's own dates
                    initCalendar();
                });
        }
